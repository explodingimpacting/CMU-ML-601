...
\subpart[1] \textbf{Select all that apply:} Which of the following scenarios are possible? 
    ...
    \CorrectChoice $\mathcal{D}_{t'+1} (i) > \mathcal{D}_{t'} (i)$ and $h_{t'}$ incorrectly classifies $x^{(i)}$
    \CorrectChoice $\mathcal{D}_{t'+1} (i) < \mathcal{D}_{t'} (i)$ and $h_{t'}$ correctly classifies $x^{(i)}$
...
You do not need to provide a line of code unless you feel that it would help you in your explanation.
    ...
    i and j are being sampled uniformly so there is no logic preventing i = j or sampling the same 
    student twice.
    \end{your_solution}
...
1. No \\
2. No \\
3. No, just recitation, some reading links given in course website and lecture notes.
