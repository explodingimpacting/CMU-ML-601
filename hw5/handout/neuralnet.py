...
def random_init(shape):
    ...
    # return unifrom distribution with given values in noted statement.
...
class SoftMaxCrossEntropy:
    ...
    def _softmax(self, z: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO: implement
        # returns probabilities or y hat
        prob = np.exp(z - np.max(z))
    ...
    def _cross_entropy(self, y: int, y_hat: np.ndarray) -> float:
        ...
        """
        # TODO: implement
        # cross entropy loss is between true label nad predicted probs
        # Debugging prints to check the shapes
        print(f"y: {y}")  
        # y_hat is (1, num_classes)
        print(f"y_hat shape: {y_hat.shape}")  

        if y_hat.ndim == 1:
            y_hat = y_hat.reshape(1, -1)

        c_e_loss = -np.log(y_hat[0, y])
    ...
    def forward(self, z: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        # use softmax and cross entropy to calculate prob and loss as stated
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> np.ndarray:
        ...
        """
        # y hat for class (predicted prob) - true labels (y_k)
        # create copy of y_hat
        # Should be (num_classes,)
        print(f"in softmax y_hat shape: {y_hat.shape}")  

        if y_hat.ndim > 1:
            y_hat = y_hat.squeeze()

        gradient = y_hat.copy()
        # applies formula from written subtracting from true class prob
        gradient[y] -= 1
...
class Sigmoid:
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO: implement 
        part_dev = dz * self.output * (1 - self.output)
...
class Linear:
    def __init__(self, input_size: int, output_size: int,
        ...
        # TODO: Initialize any additional values you may need to store for the
        #  backward pass here; gradient of loss respect to bias
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        # had shape errors from 1d to 2d
        if len(x.shape) == 1:
            x = x.reshape(1, -1) 
        ...
        # shape after adding bias
        print(f"self.x shape: {self.x.shape}")  
        # shape of weights
        print(f"weights shape: {self.weights.shape}")  

        # forward pass : z = W * x + b
        z = np.dot(self.x, self.weights.T) 
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        self.grad_weight = np.dot(dz, self.x)
        self.grad_bias = np.sum(dz, axis=0)

        # gradient input without bias
        dx = np.dot(self.weights.T, dz).squeeze()
...
class NN:
    def __init__(self, input_size: int, hidden_size: int, output_size: int,
        ...
        self.softmax_cross = SoftMaxCrossEntropy()
    ...
    def forward(self, x: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        # following off of psuedocode
        # first through hidden layer
        a_hidden = self.hidden_layer.forward(x)
        # sigmooid activation module
        z_hidden = self.sigmoid.forward(a_hidden)
        # continue forward
        a_output = self.output_layer.forward(z_hidden)
        # softmax & cross entropy modules
        y_hat, loss = self.softmax_cross.forward(a_output, y)
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> None:
        ...
        # going off of module psuedocde
        g_output = self.softmax_cross.backward(y, y_hat)
        
        # backpropogate through output
        g_hidden = self.output_layer.backward(g_output)

        # apply sigmoid
        g_a = self.sigmoid.backward(g_hidden)

        # keep backpropogating through 
        self.hidden_layer.backward(g_a)
    ...
    def compute_loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        loss = 0
        # forward function through all samples to get predicted probs
        y_hat = self.forward(X, y)[0]

        # calculate cross entropy
        for i in range(X.shape[0]):
            loss += self.softmax_cross._cross_entropy(y[i], y_hat[i])
    ...
    def train(self, X_tr: np.ndarray, y_tr: np.ndarray,
        ...
        """
        # TODO: train network
        # going off algo 3 psuedocode for SGD module
        train_losses = []
        test_losses = []

        for epoch in range(n_epochs):
            # shuffle for x & y
            X_shuffle, y_shuffle = shuffle(X_tr, y_tr, epoch)
            ...
            for i in range(X_shuffle.shape[0]):
                ...
                #update parameteres with weigh & biases
                self.step()
